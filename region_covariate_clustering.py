...
'''
Region covariate clustering code, part of MutEnricher.
...
# MAIN #
########
def covariate_cluster(cfn,weights_fn,sim_prefix,ap_iters,convits,alg,pool):
    '''
    ...
    print '\nComputing similarities by contig...'
    ...
    for c in vals_D:
        ...
        clusters_D[c] = cl
    return clusters_D
    print '\nRegion clustering finished.'

#####################
# UTILITY FUNCTIONS #
#####################
def read_covariates_file(cfn):
    '''
    ...
    for line in lines[1:]:
        ...
        if contig not in VALUES:
            VALUES[contig] = {}
            VALUES[contig]['ids'] = [l[0]]
            VALUES[contig]['values'] = []
        else:
            VALUES[contig]['ids'].append(l[0])
        vals = []
        for v in l[1:]:
            try: 
                vals.append(float(v))
