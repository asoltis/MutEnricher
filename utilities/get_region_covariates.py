...
def main():
    ...
    for r in regions:
        ...
        for rs in rs_fns:
            samp = rs[1]
            if r.repTiming[samp] != None:
                ostr.append('%0.3f'%(r.repTiming[samp]))
            else: ostr.append(str(r.repTiming[samp]))
        ostr = '\t'.join(ostr)+'\n'
        of.writelines(ostr)
    of.close()
...
def get_replication_timing_data(regions,repTime):
    '''
    Get replication timing data for region for input files in rs_fns. 
    Computes mean replication timing value in 100 kb window around region midpoint.
    '''
    for fn,name in repTime:
        tb = TabixFile(fn)
        ...
        for r in regions:
            ...
            rt_regions = tb.fetch(rstr)
            repTime = []
            for rtr in rt_regions:
                if rtr == '': continue
                repTime.append(float(rtr.split('\t')[-1]))

            if len(repTime)>0:
                r.repTiming[name] = np.mean(repTime)
                continue
            else: 
